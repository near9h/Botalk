...
```
              ...
              ▼                  ▼                  ▼            ▼             ▼
   ┌─────────────────┐ ┌─────────────────┐ ┌────────────────┐ ┌──────────┐ ┌──────────┐
   │ Postgres 16     │ │ NewAPI          │ │ MinerU         │ │ RAGFlow  │ │ zhipuai  │
   │ (pgvector)      │ │ (OpenAI 兼容)   │ │ (PDF 解析)     │ │ (可选)   │ │ (embed)  │
   │ :5432           │ │ :5000           │ │                │ │          │ │          │
   │ 持久化 + 向量   │ │ LLM 网关        │ │ 文档解析       │ │ 外部 RAG │ │ 向量 API │
   └─────────────────┘ └────────┬────────┘ └────────────────┘ └──────────┘ └──────────┘
                                │
                                ▼
                    ┌──────────────────────┐
...
| **存储 / 外部依赖层** | Postgres + NewAPI + MinerU + RAGFlow + zhipuai | 持久化 + LLM + 解析 + 向量 |
